...
if __name__ == "__main__":
    ...
    model_ckpt = "../all_code/CA-best_test_model.pth"
    ...
    # target column
    target_col = robust_df.columns[-1]
    ...
    # Build model + SNGP
    # -------------------------------------------------
    model = CARegressor(input_dim=X_threshold.shape[1])
    ...
    # -------------------------------------------------
    # Compute variance threshold (from threshold set)
    # -------------------------------------------------
    with torch.no_grad():
