...
model = CARegressor(input_dim=x_threshold_s.shape[1]).to(device)
...
print(max(np.expm1(filtered_test_pred)))
...
plt.tight_layout()
plt.savefig("../result/image_result/CA-embedding_space_OOD_vs_ID.png", dpi=600, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig("../result/image_result/CA-lda_1d_distribution.png", dpi=400)
plt.show()
...
industry_ohe_cols = [f"Industry_Type_{i}" for i in range(1, 21)]
...
print("Generate a SHAP swarm plot...")
...
plt.tight_layout()
plt.savefig("../result/image_result/CA_SHAP_Beeswarm.png", dpi=400, bbox_inches='tight')
plt.show()
...
wedges, texts = ax_inset.pie(
    top_percentages,
    labels=[None] * len(top_percentages),
    startangle=90,
    colors=colors_pie,
...
plt.tight_layout()
plt.savefig("../result/image_result/CA_SHAP_Comprehensive_Analysis.png", dpi=400, bbox_inches='tight')
plt.show()

# export SHAP analysis results to Excel
excel_path = "../result/image_result/CA_SHAP_Analysis_Results.xlsx"
...
# Printing of Feature Importance Ranking
print("=" * 60)
...
print("1. CA_SHAP_Beeswarm.png - Swarm plot")
print("2. CA_SHAP_Comprehensive_Analysis.png - Comprehensive analysis plot")
