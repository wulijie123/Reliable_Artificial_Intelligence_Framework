...
model = SNGPRegressor(input_dim=x_threshold_s.shape[1]).to(device)
...
variance_threshold = np.percentile(threshold_var, threshold)
print(f'阈值选择：{threshold}  Selected variance threshold: {variance_threshold:.6f}')
model.eval()
with torch.no_grad():
...
#t-SNE
#mlp
def extract_embedding(model, x_tensor):
    model.eval()
    with torch.no_grad():
        embedding = model.net(x_tensor)
    return embedding.cpu().numpy()


embedding_test = extract_embedding(model, x_test_tensor)  # shape: (N, 128)
...
plt.tight_layout()
plt.savefig("../result/image_result/MLP-embedding_space_OOD_vs_ID.png", dpi=600, bbox_inches='tight')
plt.show()
...
plt.legend(fontsize=16, loc='best', frameon=False, prop={'size':16,'weight':'bold'})
plt.tight_layout()
plt.savefig("../result/image_result/MLP-lda_1d_distribution.png", dpi=400)
plt.show()
...
plt.tight_layout()
plt.savefig("../result/image_result/MLP_SHAP_Beeswarm.png", dpi=400, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig("../result/image_result/MLP_SHAP_Comprehensive_Analysis.png", dpi=400, bbox_inches='tight')
plt.show()

# export SHAP analysis results to Excel
excel_path = "../result/image_result/MLP_SHAP_Analysis_Results.xlsx"
...
print("1. MLP_SHAP_Beeswarm.png - Swarm plot")
print("2. MLP_SHAP_Comprehensive_Analysis.png - Comprehensive analysis plot")
