...
if __name__ == "__main__":
    ...
    model = CARegressor(input_dim=44)
    model = convert_to_sn_my(model,
                             spec_norm_replace_list=["Linear", "Conv1d"],
                             spec_norm_bound=10)
    # Replace the output layer with a GP layer
    GP_KWARGS = {
        'num_inducing': 1024,
        'gp_scale': 0.5,
    ...
    model.load_state_dict(torch.load('./CA-80-0.82-2.pth', map_location='cuda:0'))
    ...
    def debug_nan_inf_info(arr, name):
        arr = np.array(arr)
        n_nan = np.isnan(arr).sum()
        n_inf = np.isinf(arr).sum()
        print(f"{name}: shape={arr.shape}, NaN_count={n_nan}, Inf_count={n_inf}")
        if n_nan > 0 or n_inf > 0:
            # show rows that contain NaN/Inf (up to 10)
            rows_with_bad = np.where(np.any(np.isnan(arr) | np.isinf(arr), axis=1))[0]
            print(f"  rows with bad values (first 10 indices): {rows_with_bad[:10]}")
            # if you still have original DataFrame versions, print those rows to inspect columns
        return n_nan, n_inf


    # Example use
    debug_nan_inf_info(train_X, "train_X")
    debug_nan_inf_info(calib_X, "calib_X")
    debug_nan_inf_info(test_X, "test_X")
    ...
    print(f"Average width of the 68% confidence interval: {shap_68:.4f}")
    print(f"Average width of the 95% confidence interval: {shap_95:.4f}")
    ...
    plt.ylim([-1500, 1500])
    plt.tight_layout()
    save_path = f'../result/image_result/CA-CP-conformal-uncertainty,png'
    ...
    ax.text(39, 7, "miscalc. area = {:.3f}".format(area), fontsize=16)

    plt.tight_layout()

    save_path = f'../result/image_result/CA-CP-Statistical-calibration-performance.png'
    plt.savefig(save_path, dpi=400, bbox_inches='tight')
    plt.show()

    # Modify the confidence level range to 0.5-0.99 with an interval of 0.01
    ...
    print("\n" + "=" * 80)
    print("Conformal Prediction Performance Metrics (0.0-1.0, step=0.001)")
    print("=" * 80)
    print(f"Total confidence levels evaluated: {len(performance_df)}")

    performance_df.to_csv("../result/CA-cp_performance_metrics_detailed.csv", index=False)
    print("Detailed performance metrics saved to '../result/CA-cp_performance_metrics_detailed.csv'")
    ...
    # Figure 1: Relationship between Interval Width and Confidence Level
    ...
    # Mark key points without adding text annotations
    ...
    plt.tight_layout()
    plt.savefig('../result/image_result/CA-cp_interval_width_vs_confidence.png',
                dpi=400, bbox_inches='tight', facecolor='white')
    plt.show()

    # Figure 2: Distribution of Confidence Level Deviation (10:6 Ratio)
    fig2, ax2 = plt.subplots(figsize=(10, 6))

    confidence_diff = performance_df["Confidence Difference"] * 100

    # Smooth the curve using the interpolation method
    from scipy.interpolate import CubicSpline

    # Create an interpolation function
    cs = CubicSpline(expected_conf, confidence_diff)

    # Create denser x-values for smoothing the curve
    x_smooth = np.linspace(expected_conf.min(), expected_conf.max(), 500)
    y_smooth = cs(x_smooth)

    # Fix the blank issue in the filled area - Use a single fill function
    # plot the negative value region
    ax2.fill_between(x_smooth, y_smooth, 0,
                     where=(y_smooth <= 0),
                     color='#F25F5C', alpha=0.5, label='Under-confident')

    # plot the positive value region
    ...
    plt.tight_layout()
    plt.savefig('../result/image_result/CA-cp_confidence_difference_distribution.png',
                dpi=400, bbox_inches='tight', facecolor='white')
    plt.show()
