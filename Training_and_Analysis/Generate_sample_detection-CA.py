...
model = CARegressor(input_dim=x_train_s.shape[1]).to(device)
...
#plt.title('CA Model - OOD Detection ROC Curves', fontsize=18)
plt.legend(loc='lower right', frameon=False, fontsize=18)

plt.tight_layout()
plt.savefig("../result/image_result/CA-ood_roc_comparison_combined.png", dpi=600, bbox_inches='tight')
plt.show()

results_df.to_excel("../result/data_result/CA_ood_detection_metrics.xlsx", index=False)
print("指标结果已保存到: ../result/data_result/CA_ood_detection_metrics.xlsx")
...
plt.savefig(
    "../result/image_result/CA_ood_PR_comparison_combined.png",
    dpi=800,
    bbox_inches='tight'
)
plt.show()
...
pr_df.to_excel(
    "../result/data_result/CA_ood_PR_curve.xlsx",
    index=False
)
