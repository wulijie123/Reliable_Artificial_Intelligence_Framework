...
class _SpectralNorm(Module):
    ...
    def forward(self, weight: torch.Tensor) -> torch.Tensor:
        if weight.ndim == 1:
            ...
            w_norm = torch.where((self.spec_norm_bound / sigma) < 1, (self.spec_norm_bound / sigma) * weight, weight)
            return w_norm
...
def spectral_norm(
    ...
    Args:
        ...
        spec_norm_bound (float, optional): the upper bound of the spectral
            noramlization. Default: ``1.0``.
    ...
    parametrize.register_parametrization(
        module, name, _SpectralNorm(weight, n_power_iterations, dim, eps, spec_norm_bound)
    )
    return module

if __name__ == "__main__":
    snm = spectral_norm(torch.nn.Linear(20, 40), spec_norm_bound=0.7)
    # a = torch.nn.Conv2d(1, 3, 2)
    # print(a.weight)
    # snm = spectral_norm(a, spec_norm_bound=0.5)
    # print(snm.weight.shape)
    # print("snm.weight.", snm.weight)
    # print("smm", torch.linalg.matrix_norm(snm.weight, dim=(2, 3)))